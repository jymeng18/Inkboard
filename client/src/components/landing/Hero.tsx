  ...
  return (
    <section className="relative mb-20 text-center">
      <h1 className="relative mb-6 font-display text-5xl leading-[0.95] sm:text-6xl md:text-7xl">
        Unleash the <span className="inline-block rotate-2 text-primary italic">kaos.</span>
        <br />
      ...
      <p className="mx-auto mb-9 max-w-xl font-body text-base text-on-background/70 md:text-lg">
        ...
        <span className="font-bold text-on-background">Operational Transformations</span>. No
        conflicts, just pure creative flow.
      </p>
        ...
        <div className="flex -space-x-3">
          {AVATAR_IMAGES.map((src, i) => (
            <RemoteImage
              ...
              fallback={<Avatar index={i} className="size-10 rounded-full border-[3px] border-outline" />}
            />
          ))}
        </div>
        <p className="font-handwriting text-base text-on-background/60">
          <span className="font-bold text-primary">Rin</span> and 84 others are sketching now.
        </p>
      </div>
    </section>
  )
